<template>
  <div class="timeline">
    ...
    <div class="item" v-for="item in itemList" :key="item.id">
      <div class="item-year" v-if="checkYear(item.updateTime)" >
        <span class="iconfont">&#xe503;</span>{{getYear(item.updateTime)}}
      </div>
      <div class="item-abstract" @click="handleToArticle(item.id)">
        <span class="item-date">{{getMonthDate(item.updateTime)}}</span>
